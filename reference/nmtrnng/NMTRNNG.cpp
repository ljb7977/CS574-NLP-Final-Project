...
void NMTRNNG::biEncode(const NMTRNNG::Data *data,
    ...
    for (int i = 0; i < arg.srcLen; ++i) {
        if (i == 0) {
            ...
            this->encRev.forward(this->sourceEmbed.col(data->src[length - i]), arg.encRevState[i - 1],
                                 arg.encRevState[i]);
        }
...
void NMTRNNG::biEncoderBackward2(const NMTRNNG::Data *data,
    ...
    for (int i = arg.srcLen - 1; i >= 0; --i) {
        this->encRev.backward2(arg.encRevState[i], grad.lstmSrcRevGrad,
                               this->sourceEmbed.col(data->src[arg.srcLen - i - 1]), LSTM::WXI);
    }
    for (int i = arg.srcLen - 1; i >= 1; --i) {
        this->encRev.backward2(arg.encRevState[i], grad.lstmSrcRevGrad,
                               this->sourceEmbed.col(data->src[arg.srcLen - i - 1]), LSTM::WXF);
    }
    for (int i = arg.srcLen - 1; i >= 0; --i) {
        this->encRev.backward2(arg.encRevState[i], grad.lstmSrcRevGrad,
                               this->sourceEmbed.col(data->src[arg.srcLen - i - 1]), LSTM::WXO);
    }
    for (int i = arg.srcLen - 1; i >= 0; --i) {
        this->encRev.backward2(arg.encRevState[i], grad.lstmSrcRevGrad,
                               this->sourceEmbed.col(data->src[arg.srcLen - i - 1]), LSTM::WXU);
    }
    for (int i = arg.srcLen - 1; i >= 1; --i) {
...
void NMTRNNG::translate(NMTRNNG::Data *data,
    ...
    for (int i = 0; i < maxLength; ++i) {
        for (int j = 0; j < beamSize; ++j) {
            ...
            if (i == 0) {
                ...
                decInitStateEnd.segment(this->hiddenEncDim, this->hiddenEncDim).noalias() = arg.encRevState[arg.srcLen -
                                                                                                            1]->h;
                ...
                this->dec.forward(this->targetEmbed.col(arg.candidate[j].generatedTarget[i - 1]),
    ...
    if (train) {
        ...
        for (int i = 0; i < showNum; ++i) {
            ...
            for (auto it = arg.candidate[i].generatedTarget.begin();
                 it != arg.candidate[i].generatedTarget.end(); ++it) {
...
void NMTRNNG::translateWithAction(NMTRNNG::Data *data,
    ...
    arg.candidate[0].utEnd.segment(this->hiddenDim,
                                   this->hiddenDim).noalias() = arg.candidate[0].outBufState[arg.candidate[0].k]->h;
    ++arg.candidate[0].k;
    ...
    for (int i = 0; i < maxLength; ++i) {
        ...
        for (int j = 0, row, col; j < beamSize; ++j) {
            ...
            candidateTmp[j].utEnd.segment(this->hiddenDim,
                                          this->hiddenDim).noalias() = candidateTmp[j].outBufState[candidateTmp[j].k]->h;
            ++candidateTmp[j].k;
        ...
        for (int j = 0, row, col; j < beamSize; j++) {
            ...
            while (!shift) {
                ...
                arg.candidate[j].utEnd.segment(this->hiddenDim * 2,
                                               this->hiddenActDim).noalias() = arg.candidate[j].curAct.h;
                // forward
                ...
                if (this->actionVoc.tokenList[row]->action == 0) { // shift
                    ...
                    if (rightNum < arg.candidate[j].tgtLen &&
                        leftNum < arg.candidate[j].tgtLen) { // word embedding & word embeddding
                        this->compositionFunc(
                                arg.candidate[j].embedVec[arg.candidate[j].phraseNum - arg.candidate[j].tgtLen],
                    ...
                    this->outBuf.forward(
                            arg.candidate[j].embedVec[arg.candidate[j].phraseNum - arg.candidate[j].tgtLen],
                            arg.candidate[j].outBufState[top],
                            arg.candidate[j].outBufState[arg.candidate[j].k]); // (xt, prev, cur)
                    arg.candidate[j].embedStack.push(arg.candidate[j].phraseNum);
                    ++arg.candidate[j].phraseNum;
                    arg.candidate[j].utEnd.segment(this->hiddenDim,
                                                   this->hiddenDim).noalias() = arg.candidate[j].outBufState[arg.candidate[j].k]->h;
                    ++arg.candidate[j].k;
                    ...
                    if (rightNum < arg.candidate[j].tgtLen &&
                        leftNum < arg.candidate[j].tgtLen) { // word Embed & word Embed
                        this->compositionFunc(
                                arg.candidate[j].embedVec[arg.candidate[j].phraseNum - arg.candidate[j].tgtLen],
...
void NMTRNNG::train(NMTRNNG::Data *data,
    ...
    for (int i = arg.actLen - 1; i >= 0; --i, --k) {
        ...
        if (this->actionVoc.tokenList[actNum]->action == 0) { // 0: Shift
            ...
            this->stildeAffine.backward1(arg.s_tilde[j], arg.del_stilde[j], arg.del_stilde[j], arg.del_stildeEnd,
                                         grad.stildeAffineGrad);
            arg.decState[j]->delh.noalias() += arg.del_stildeEnd.segment(0, this->hiddenDim);

            // del_contextSeq
            for (int j2 = 0; j2 < arg.srcLen; ++j2) { // Seq
                arg.del_alphaSeqTmp =
                        arg.alphaSeq.coeff(j2, j) * arg.del_stildeEnd.segment(this->hiddenDim, this->hiddenEncDim * 2);
                arg.encState[j2]->delh.noalias() += arg.del_alphaSeqTmp.segment(0, this->hiddenEncDim);
                arg.encRevState[arg.srcLen - j2 - 1]->delh.noalias() += arg.del_alphaSeqTmp.segment(this->hiddenEncDim,
                                                                                                    this->hiddenEncDim);
                arg.del_alphaSeq.coeffRef(j2, 0) = arg.del_stildeEnd.segment(this->hiddenDim,
                                                                             this->hiddenEncDim * 2).dot(
                        arg.biEncState[j2]);
            }
            arg.del_alignScore = arg.alphaSeq.col(j).array() * (arg.del_alphaSeq.array() - arg.alphaSeq.col(j).dot(
                    arg.del_alphaSeq)); // X.array() - scalar; np.array() -= 1

            for (int j2 = 0; j2 < arg.srcLen; ++j2) {
                arg.del_WgeneralTmp = this->Wgeneral.transpose() * arg.decState[j]->h;
                arg.encState[j2]->delh.noalias() +=
                        arg.del_WgeneralTmp.segment(0, this->hiddenEncDim) * arg.del_alignScore.coeff(j2, 0);
                arg.encRevState[arg.srcLen - j2 - 1]->delh.noalias() +=
                        arg.del_WgeneralTmp.segment(this->hiddenEncDim, this->hiddenEncDim) *
                        arg.del_alignScore.coeff(j2, 0);

                arg.decState[j]->delh.noalias() +=
                        (this->Wgeneral * arg.biEncState[j2]) * arg.del_alignScore.coeff(j2, 0);
            ...
            this->embedVecAffine.backward(arg.embedVecEnd[phraseNum - arg.tgtLen], arg.embedVec[phraseNum - arg.tgtLen],
                                          arg.del_embedVec[phraseNum - arg.tgtLen], arg.del_embedVecEnd,
                                          grad.embedVecAffineGrad);
            ...
            if (rightNum < arg.tgtLen && leftNum < arg.tgtLen) { // word embedding & word embedding
                ...
                if (grad.targetEmbed.count(tgtLeftNum)) {
                    grad.targetEmbed.at(tgtLeftNum).noalias() += arg.del_embedVecEnd.segment(this->inputDim,
                                                                                             this->inputDim);
                } else {
                    grad.targetEmbed[tgtLeftNum].noalias() = arg.del_embedVecEnd.segment(this->inputDim,
                                                                                         this->inputDim);
                }
            ...
            if (grad.actionEmbed.count(data->action[i - 1])) {
                grad.actionEmbed.at(data->action[i - 1]).noalias() += arg.del_embedVecEnd.segment(this->inputDim * 2,
                                                                                                  this->inputActDim);
            } else {
                grad.actionEmbed[data->action[i - 1]] = arg.del_embedVecEnd.segment(this->inputDim * 2,
                                                                                    this->inputActDim);
            }
            ...
            if (rightNum < arg.tgtLen && leftNum < arg.tgtLen) { // Word Embed & word Embed
                ...
                if (grad.targetEmbed.count(tgtRightNum)) {
                    grad.targetEmbed.at(tgtRightNum).noalias() += arg.del_embedVecEnd.segment(this->inputDim,
                                                                                              this->inputDim);
                } else {
                    grad.targetEmbed[tgtRightNum].noalias() = arg.del_embedVecEnd.segment(this->inputDim,
                                                                                          this->inputDim);
                }
                ...
                if (arg.del_embedVec.count(rightNum)) {
                    arg.del_embedVec.at(rightNum).noalias() += arg.del_embedVecEnd.segment(this->inputDim,
                                                                                           this->inputDim);
                } else {
            ...
            if (grad.actionEmbed.count(data->action[i - 1])) {
                ...
                grad.actionEmbed[data->action[i - 1]].noalias() = arg.del_embedVecEnd.segment(this->inputDim * 2,
                                                                                              this->inputActDim);
            }
    ...
    arg.encRevState[arg.srcLen - 1]->delh.noalias() += arg.del_decInitStateEnd.segment(this->hiddenEncDim,
                                                                                       this->hiddenEncDim);
    ...
    arg.encRevState[arg.srcLen - 1]->delh.noalias() += arg.del_encStateEnd.segment(this->hiddenEncDim,
                                                                                   this->hiddenEncDim);
    ...
    arg.encRevState[arg.srcLen - 1]->delh.noalias() += arg.del_outBufInitStateEnd.segment(this->hiddenEncDim,
                                                                                          this->hiddenEncDim);
...
void NMTRNNG::calculateAlpha(NMTRNNG::ThreadArg &arg,
    ...
    arg.alphaSeq.col(colNum) = arg.alphaSeq.col(
            colNum).array().exp(); // exp() operation for all elements; np.exp(alphaSeq)
...
bool NMTRNNG::trainOpenMP(NMTRNNG::Grad &grad) {
    ...
    if (args.empty()) {
        ...
        for (int i = 0, step = this->trainData.size() / this->miniBatchSize; i < step; ++i) {
            miniBatch.push_back(std::pair<int, int>(i * this->miniBatchSize,
                                                    (i == step - 1 ? this->trainData.size() - 1 :
                                                     (i + 1) * this->miniBatchSize - 1)));
        }
    }
...
#pragma omp parallel for num_threads(this->threadNum) schedule(dynamic) shared(args, grad, grads)
        ...
        lr = (gradNorm > this->clipThreshold ? this->clipThreshold * this->learningRate / gradNorm
                                             : this->learningRate);
    ...
    std::cout << "Training time for this epoch: "
              << (std::chrono::duration_cast<std::chrono::seconds>(endTmp - startTmp).count()) / 60.0 << "min."
              << std::endl;
...
void NMTRNNG::loadCorpus(const std::string &src,
    ...
    for (std::string line; std::getline(ifsSrc, line);) {
        ...
        for (auto it = tokens.begin(); it != tokens.end(); ++it) {
            data.back()->src.push_back(
                    sourceVoc.tokenIndex.count(*it) ? sourceVoc.tokenIndex.at(*it) : sourceVoc.unkIndex);
        }
    ...
    for (std::string line; std::getline(ifsTgt, line);) {
        ...
        for (auto it = tokens.begin(); it != tokens.end(); ++it) {
            data[numLine]->tgt.push_back(
                    targetVoc.tokenIndex.count(*it) ? targetVoc.tokenIndex.at(*it) : targetVoc.unkIndex);
        }
