...
void AttentionBiEncDec::train(AttentionBiEncDec::Data *data,
    ...
    for (int i = arg.tgtLen - 1; i >= 0; --i) {
        ...
        for (int j = 0; j < arg.srcLen; ++j) { // Seq
            arg.del_alphaSeqTmp =
                    arg.alphaSeq.coeff(j, i) * arg.del_stildeEnd.segment(this->hiddenDim, this->hiddenEncDim * 2);
            arg.encState[j]->delh.noalias() += arg.del_alphaSeqTmp.segment(0, this->hiddenEncDim);
            arg.encRevState[arg.srcLen - j - 1]->delh.noalias() += arg.del_alphaSeqTmp.segment(this->hiddenEncDim,
                                                                                               this->hiddenEncDim);
            arg.del_alphaSeq.coeffRef(j, 0) = arg.del_stildeEnd.segment(this->hiddenDim, this->hiddenEncDim * 2).dot(
                    arg.biEncState[j]);
        }
        arg.del_alignScore = arg.alphaSeq.col(i).array() * (arg.del_alphaSeq.array() - arg.alphaSeq.col(i).dot(
                arg.del_alphaSeq)); // X.array() - scalar; np.array() -= 1

        for (int j = 0; j < arg.srcLen; ++j) {
            arg.del_WgeneralTmp = this->Wgeneral.transpose() * arg.decState[i]->h;
            arg.encState[j]->delh.noalias() +=
                    arg.del_WgeneralTmp.segment(0, this->hiddenEncDim) * arg.del_alignScore.coeff(j, 0);
            arg.encRevState[arg.srcLen - j - 1]->delh.noalias() +=
                    arg.del_WgeneralTmp.segment(this->hiddenEncDim, this->hiddenEncDim) *
                    arg.del_alignScore.coeff(j, 0);
    ...
    // Decoder -> Encoder
    this->decInitAffine.backward(arg.decInitStateEnd, arg.decState[0]->h, arg.decState[0]->delh,
